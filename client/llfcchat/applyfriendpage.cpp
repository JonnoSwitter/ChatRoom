    ...
    ui(new Ui::ApplyFriendPage)
{
    ...
    loadApplyList();
    //接受tcp传递的authrsp信号处理
    connect( Singleton<TcpMgr>::GetInstance().get(), &TcpMgr::sig_auth_rsp, this, &ApplyFriendPage::slot_auth_rsp);
}
...
{
    //添加好友申请
    auto apply_list = Singleton<UserMgr>::GetInstance()->GetApplyList();
